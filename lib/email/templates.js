...
function linkEmail({ subject, title, intro, buttonLabel, link, notes }) {
  ...
  const body =
    paragraph(intro) +
    `<table role="presentation" cellpadding="0" cellspacing="0" style="margin:0 0 20px"><tr><td style="border-radius:999px;background:${COLORS.ink}">
      <a href="${safeLink}" style="display:inline-block;padding:13px 28px;font-size:15px;font-weight:600;color:#ffffff;text-decoration:none;border-radius:999px">${escapeHtml(buttonLabel)}</a>
    </td></tr></table>` +
    notes.map(note).join("") +
...
/**
 * One-time code typed on the sign-up page. The code is also in the subject so
 * it shows in the phone notification and mail apps can detect it.
 */
export function signupCodeEmail(code) {
  const safeCode = escapeHtml(code);
  const intro = "멘토링 예약 회원가입 화면에 아래 인증 코드를 입력해주세요.";
  const notes = [
  ...
  const body =
    ...
    `<p style="margin:0 0 20px;font-size:12px;line-height:1.6;color:${COLORS.faint};text-align:center">코드를 눌러 선택하고 복사한 뒤 회원가입 화면에 붙여넣어 주세요.</p>` +
    notes.map(note).join("");
  return {
    subject: `[멘토링 예약] 인증 코드 ${code}`,
    html: layout({ title: "이메일 인증 코드", preheader: `인증 코드 ${code}`, body }),
    text: [intro, `인증 코드: ${code}`, ...notes].join("\n\n"),
    attachments: [LOGO_ATTACHMENT],
  };
}
